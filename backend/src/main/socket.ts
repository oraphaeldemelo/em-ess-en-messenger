...
import {
    ...
    leaveRoomSchema,
    sendMessageSchema,
} from '@/interfaces/socket/socketSchemas';
...
import type { JoinRoomPayload, LeaveRoomPayload, SendMessagePayload } from '../types/socketEvents';
...
export function buildSocketServer(httpServer: HttpServer): AppSocketServer {
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('join-room', (receivedPayload) => {
            const payload = validateSocketPayload<JoinRoomPayload>(joinRoomSchema, receivedPayload);
